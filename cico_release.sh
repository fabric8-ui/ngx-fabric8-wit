...
function create_merge_PR {
    ...
    current_version=$( grep ngx-fabric8-wit package.json \
        | awk -F: '{ print $2 }' \
        | sed 's/[",]//g' \
        | tr -d '[:space:]' )
    echo "New fabric8-wit version:" new_version
    echo "Current fabric8-wit version:" $current_version
    if [ "new_version" == "$current_version" ]; then
        echo "Skippping as ngx-fabric8-wit is already on version new_version"
        exit 0
    fi
    ...
    message="fix(version): update package.json ngx-fabric8-wit to ${new_version}"
